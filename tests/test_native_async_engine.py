...
from sqlalchemy_neon.neon_http_client import QueryResult
from sqlalchemy_neon.types import PostgresOID
from tests.integration.models import User, Post, Comment
...
async def test_native_engine_execute_forwards_to_client(mock_connection_string: str):
    class FakeClient:
        ...
        async def query(self, sql, params, options=None):
            ...
            return QueryResult(rows=[{"v": 1}], fields=[{"name": "v"}], row_count=1, command="SELECT")
...
async def test_native_engine_transaction_forwards_compiled_statements(
    ...
    class FakeClient:
        ...
        async def transaction(self, queries):
            ...
            return [
                QueryResult(rows=[[1]], fields=[{"name": "v"}], row_count=1, command="SELECT", row_as_array=True),
                QueryResult(rows=[[2]], fields=[{"name": "v"}], row_count=1, command="SELECT", row_as_array=True),
            ]
...
async def test_native_engine_hydrates_loader_option_relationships(
    ...
    class FakeClient:
        async def query(self, sql, params, options=None):
            ...
            if (
                "from public.posts join public.users" in sql_l
                or (
            ...
            if (
                "from public.posts join public.post_tags" in sql_l
                or (
                    "from (select public.posts.id as __pid" in sql_l
            ...
            if (
                "from public.posts join public.comments" in sql_l
                or (
                    "from (select public.posts.id as __pid" in sql_l
...
async def test_native_engine_loads_sibling_relationships_concurrently(
    ...
    stmt = sa.select(Post).where(Post.id == 1).options(
